...
"""
prompts.py – All Groq prompt templates live here.
...
A CSV or Excel file is uploaded. Do not assume the domain.

First analyze the file silently:
- Infer the grain (what one row is), key dimensions, and the most useful metrics.
- If a clear outcome column exists (status, churn, converted, default, target, yes/no, 0/1), use it as the primary KPI. If not, pick the strongest business measure (revenue, count, amount, rate).
- Clean obvious type issues, handle missing values, and ignore ID-only columns.
- Produce 4–6 real KPIs, 3–5 breakdowns that actually move the metric, and 3–5 insight segments. Use only calculated numbers.

Then generate one 16:9 Power BI-style dashboard image from those results:
Dark navy background, teal + coral accents, rounded cards, left slicer pane.
Title from the dataset topic. KPI row on top. Mix of bars, combo/trend, donut, and a small insight table or persona/segment cards.
Readable labels, clean enterprise look, no watermark, no fake logos, no invented figures.
"""
