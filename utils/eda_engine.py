...
def sanitize_plan(
    ...
    for kpi in plan.get("kpis", []):
        ...
        if col not in valid_cols:
            logger.warning(
                "KPI dropped – column '%s' not in dataframe",
                col,
            )
            continue
    ...
    for chart in plan.get("charts", []):
        ...
        if ctype not in ALLOWED_CHART_TYPES:
            logger.warning(
                "Chart dropped – unsupported type '%s'",
                ctype,
            )
            continue
        ...
        for key in cols_to_check:
            ...
            if val is not None and val not in valid_cols:
                logger.warning(
                    "Chart dropped – column '%s' (key=%s) not in dataframe",
                    val,
                    key,
                )
                skip = True
        ...
        if ctype in needs_x and chart.get("x") is None:
            logger.warning(
                "Chart dropped – 'x' is required for type '%s'",
                ctype,
            )
            continue
...
def compute_kpis(
    ...
    for kpi in kpis:
        ...
        try:
            ...
            logger.error(
                "KPI computation failed for %s/%s: %s",
                col,
                agg,
                exc,
            )
        ...
        # Compute a meaningful delta indicator
        delta, delta_label = _compute_kpi_delta(
            df,
            col,
            agg,
            value,
...
def _compute_kpi_delta(
    ...
    - For max/min: delta = % difference from mean
    - For others: no delta
    """
    if value is None or not pd.api.types.is_numeric_dtype(
        df.get(col, pd.Series())
    ):
        return None, ""
    ...
    try:
        ...
        if agg == "mean":
            ...
            if median_val != 0:
                pct_diff = round(
                    ((value - median_val) / abs(median_val)) * 100,
                    1,
                )
            ...
            if mean_val != 0:
                pct_diff = round(
                    ((value - mean_val) / abs(mean_val)) * 100,
                    1,
                )
    ...
    if y and x:
        ...
        if nunique_x > _HIGH_CARDINALITY_THRESHOLD:
            ...
            if pd.api.types.is_numeric_dtype(
                ...
                logger.info(
                    "Bar chart auto-aggregated: %d → %d groups",
                    nunique_x,
                    len(plot_df),
                )

        elif color and pd.api.types.is_numeric_dtype(
            plot_df.get(y, pd.Series())
        ):
...
def _build_line_chart(
    ...
    if pd.api.types.is_datetime64_any_dtype(
        plot_df.get(x, pd.Series())
    ):
        plot_df = plot_df.sort_values(x)

        if y and pd.api.types.is_numeric_dtype(
            plot_df.get(y, pd.Series())
        ):
            # Determine resample frequency based on date span
            span = (
                plot_df[x].max() - plot_df[x].min()
            ).days
            ...
            logger.info(
                "Line chart resampled to %s (%d points)",
                freq_label,
                len(plot_df),
            )
...
def _build_pie_chart(
    ...
    if len(value_counts) > 15:
        ...
        other_count = (
            value_counts.iloc[14:]["count"].sum()
        )
...
def _build_heatmap(
    ...
    if numeric_df.shape[1] < 2:
        logger.warning(
            "Heatmap skipped – fewer than 2 numeric columns"
        )
        return None
